...
(function () {
    ...
    function F(F) {
        ...
        if (F.substring(0, 1) != "/" && F.indexOf("://") == -1) {
            ...
            if ($ != "") for (H = 0; H < $.split("/").length; H++) B += "../";
            if (D != "") B += D + "/";
            F = location.href.substring(0, location.href.lastIndexOf("/") + 1) + B + F
        }
        _.$dpPath = F
    }

    function M(B, $, D) {
        var A = X[O], E = A[C]("HEAD").item(0), _ = A.createElement("link");
        _.href = B;
        _.rel = "stylesheet";
        _.type = "text/css";
        if ($) _.title = $;
    ...
    function D($) {
        ...
        if ($dp && !$dp.eCont && $dp.dd && Q($dp.dd) == "block" && src != $dp.el) $dp.cal.close()
    }
    ...
    function U(E, _) {
        ...
        if (_) {
            ...
            if (!E.el || E.el && E.el.disabled || (E.el == $dp.el && Q($dp.dd) != "none" && $dp.dd.style.left != "-1970px")) return;
            K(E)
        }
